...
setTimeout(() => {
  document.getElementsByTagName("body")[0].style.transition = "background 0.7s";
...
function allChallengeCompleteChecker(delay) {
  ...
  if (allChallengeComplete) {
    setTimeout(() => {
      document.getElementById("complete-challenges").style.display = "block";
...
function endGame(i, j, diagonal, winner) {
  ...
  if (challengesCanRun) {
    ...
    if (gameMode == "w-friend") {
      if (XOMatrixChecker(["X", "O", "X", "O", "X", "O", "X", "O", "X"])) {
...
document
  ...
  .addEventListener("click", function () {
    if (runChallengesInp.checked) {
      ...
      document.getElementById("show-challenges-label").style.opacity = "1";
      showChallengesInp.checked = true;
    } else {
